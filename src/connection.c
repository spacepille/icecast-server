...
static matchfile_t *banned_ip, *allowed_ip, *proxy_ip;
...
static void _apply_client_proxy_ip(client_t *client)
{
    const char *str = httpp_getvar(client->parser, "x-forwarded-for");
    if (!str) {
        /* no ip was forwarded */
        ICECAST_LOG_DEBUG("No forwarded ip found");
        return;
    }

    ICECAST_LOG_DEBUG("Found forwarded ip \"%s\"", str);

    ice_config_t *config = config_get_config();
    int found = 0;

    if (config->forwarded_for_ip) {
        /* icecast-hk style */
        if (strcmp(client->con->ip, config->forwarded_for_ip) == 0) {
            found = 1;
        }
    }
    
    if (!found && config->proxy_file) {
        /* search the client ip in the proxy ip file */
        if (matchfile_match(proxy_ip, client->con->ip)) {
            found = 1;
        }
    }

    if (!found) {
        /* client ip did not match */
        ICECAST_LOG_DEBUG("Proxy ip not set or not matching");
        return;
    }
    ...
    ICECAST_LOG_DEBUG("Apply forwarded ip \"%s\" to client \"%s\"", ip, client->con->ip);
...
void connection_setup_sockets (ice_config_t *config)
{
    ...
    if (config->proxy_file) {
        matchfile_release(proxy_ip);
        proxy_ip = matchfile_new(config->proxy_file);
    }
