...
void config_clear(ice_config_t *c)
{
    ...
    if (c->proxy_file)      xmlFree(c->proxy_file);
    ...
    if (c->forwarded_for_ip)
        xmlFree(c->forwarded_for_ip);
                         ...
                         ice_config_t  *configuration)
{
    ...
    do {
        ...
        } else if (xmlStrcmp(node->name, XMLSTR("proxy-ip")) == 0) {
            if (configuration->proxy_file)
                xmlFree(configuration->proxy_file);
            configuration->proxy_file = (char *)xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
        } else if (xmlStrcmp(node->name, XMLSTR("x-forwarded-for")) == 0) {
            if (configuration->forwarded_for_ip)
                xmlFree(configuration->forwarded_for_ip);
            configuration->forwarded_for_ip = (char *)xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
